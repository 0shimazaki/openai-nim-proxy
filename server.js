...
const MODEL_MAPPING = {
  ...
  'claude-3-opus': 'stepfun-ai/step-3.7-flash',
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    // Destructure incoming parameters sent by frontends like Chub/SillyTavern
    const { 
      model, 
      messages, 
      temperature, 
    ...
    // Quick safety guard to prevent crashing if messages array is missing
    if (!messages || !Array.isArray(messages)) {
    ...
    // ⚡ MODEL SPECIFIC ROLEPLAY TUNING FOR DEEPSEEK V4 FLASH
    if (nimModel === 'deepseek-ai/deepseek-v4-flash') {
      // Set optimized fallback sampler settings if frontend fields arrive blank
      if (finalTemperature === undefined) finalTemperature = 1.05;
      if (finalTopP === undefined) finalTopP = 0.92;
      if (finalFrequencyPenalty === undefined) finalFrequencyPenalty = 0.15;
      if (finalPresencePenalty === undefined) finalPresencePenalty = 0.05;

      // Disable analytical thinking loops completely and inject repetition damping
      finalExtraBody = {
        repetition_penalty: 1.04,
        chat_template_kwargs: { thinking: false }
      };

      // Automate negative style guardrail injection to enforce concrete prose
      const styleGuardrail = `\n\n[Writing Style Instructions: Write with concrete, visceral actions rather than abstract over-analysis. Avoid internal indecision loops, rhetorical questions, and safe antitheses (e.g., "You are either a saint or a monster," "Time seemed to both stop and fly"). Do not narrate what a character 'might' do or 'hasn't decided' to do—commit to immediate, observable actions, dialogue, and environment changes.]`;
      ...
      // Default temperature standard for other models
      if (finalTemperature === undefined) finalTemperature = 0.6;
    }
...
// Run listener unconditionally so Vercel can bind the server instance cleanly
app.listen(PORT, () => {
...
// Keep module exports attached for clean deployment architecture
module.exports = app;
    status: 'ok', 
    service: 'OpenAI to NVIDIA NIM Proxy', 
    reasoning_display: SHOW_REASONING,
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    // Destructure all incoming sampling variables sent by the frontend
    const { 
      model, 
      messages, 
      temperature, 
    ...
    // Prepare parameter overrides and dynamic variables
    let finalMessages = [...messages];
    let finalTemperature = temperature;
    let finalTopP = top_p;
    let finalFrequencyPenalty = frequency_penalty;
    ...
    // ⚡ CUSTOM CONFIGURATION TARGET FOR DEEPSEEK V4 FLASH
    if (nimModel === 'deepseek-ai/deepseek-v4-flash') {
      // Apply the fallback sampling stack if parameters aren't supplied by the frontend
      if (finalTemperature === undefined) finalTemperature = 1.05;
      if (finalTopP === undefined) finalTopP = 0.92;
      if (finalFrequencyPenalty === undefined) finalFrequencyPenalty = 0.15;
      if (finalPresencePenalty === undefined) finalPresencePenalty = 0.05;

      // Force thinking mode OFF for clean immersion and inject repetition penalty
      finalExtraBody = {
        repetition_penalty: 1.04,
        chat_template_kwargs: { thinking: false }
      };

      // Automate negative style guardrail injection to stop loop patterns
      const styleGuardrail = `\n\n[Writing Style Instructions: Write with concrete, visceral actions rather than abstract over-analysis. Avoid internal indecision loops, rhetorical questions, and safe antitheses (e.g., "You are either a saint or a monster," "Time seemed to both stop and fly"). Do not narrate what a character 'might' do or 'hasn't decided' to do—commit to immediate, observable actions, dialogue, and environment changes.]`;
      ...
      // Default temperature fallback if not specified for general models
      if (finalTemperature === undefined) finalTemperature = 0.6;
    }
...
// Guard the listener so it doesn't cause execution halts during Vercel serverless initiation
...
// Export the application layer for Vercel's serverless pipeline handler
module.exports = app;
    status: 'ok', 
    service: 'OpenAI to NVIDIA NIM Proxy', 
    reasoning_display: SHOW_REASONING,
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    // Set up default parameters vs overridden structures
    let finalMessages = [...messages];
    let finalTemperature = temperature;
    let finalTopP = top_p;
    let finalFrequencyPenalty = frequency_penalty;
    ...
    // ⚡ MODEL SPECIFIC ROLEPLAY TUNING FOR DEEPSEEK V4 FLASH
    if (nimModel === 'deepseek-ai/deepseek-v4-flash') {
      // Fallback to our calibrated sampling parameters if frontend doesn't supply them
      if (finalTemperature === undefined) finalTemperature = 1.05;
      if (finalTopP === undefined) finalTopP = 0.92;
      if (finalFrequencyPenalty === undefined) finalFrequencyPenalty = 0.15;
      if (finalPresencePenalty === undefined) finalPresencePenalty = 0.05;

      // Force thinking OFF to avoid analytical loops/clichés, and add repetition_penalty
      extraBody = {
        repetition_penalty: 1.04,
        chat_template_kwargs: { thinking: false }
      };

      // Automate negative system style injection
      const styleGuardrail = `\n\n[Writing Style Instructions: Write with concrete, visceral actions rather than abstract over-analysis. Avoid internal indecision loops, rhetorical questions, and safe antitheses (e.g., "You are either a saint or a monster," "Time seemed to both stop and fly"). Do not narrate what a character 'might' do or 'hasn't decided' to do—commit to immediate, observable actions, dialogue, and environment changes.]`;
      ...
      // General fallbacks for standard technical/chat models
      finalTemperature = finalTemperature || 0.6;
...
app.listen(PORT, () => {
  ...
  console.log(`Health check: http://localhost:${PORT}/health`);
  console.log(`Reasoning display: ${SHOW_REASONING ? 'ENABLED' : 'DISABLED'}`);
  console.log(`Thinking mode (Global Default): ${ENABLE_THINKING_MODE ? 'ENABLED' : 'DISABLED'}`);
});
    status: 'ok', 
    service: 'OpenAI to NVIDIA NIM Proxy', 
    reasoning_display: SHOW_REASONING,
