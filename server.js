...
// regex removal handles normal, well-formed <think>...</think> pairs.
//
// Some models (we've seen this on Gemma 4 and DeepSeek V4) have their
// "<think>" opening silently added by NVIDIA's server-side prompt template
// rather than generated by the model itself — so it never actually appears
// in the streamed text. Only the model's own "</think>" closing tag shows
// up. If that happens, everything before that stray "</think>" was reasoning
// too, even though it never got wrapped in a visible opening tag.
...
//
// It also handles the same "phantom opening tag" case as
// stripThinkTagsFromText above: some models get their "<think>" silently
// added by NVIDIA's server-side prompt template instead of generating it
// themselves, so only the closing "</think>" ever shows up in the stream.
// Since we can't un-send text once it's already been streamed to the user,
// the fix is to hold back the first bit of every response (up to
// leadBufferLimit characters) until we can tell whether a stray "</think>"
// is coming. Once that's decided — either way — buffering stops and
// everything streams through normally with no added delay.
function makeThinkStripper(leadBufferLimit = 500) {
  ...
  function processTagged(chunkText) {
  ...
  function flush() {
    // Called when the upstream stream ends. If we were still undecided
    // (buffered text never reached the tag or the length threshold),
    // there's no more data coming — release it as normal content now,
    // otherwise short replies would get held back forever.
    let output = '';
    if (!leadResolved) {
      leadResolved = true;
      const buffered = leadBuffer;
      leadBuffer = '';
      output += processTagged(buffered);
    }
    if (carry) {
      // A tag fragment that never completed (e.g. stream cut off mid-tag).
      // Nothing more is coming to finish it, so emit it as plain text.
      output += carry;
      carry = '';
    }
    return output;
  ...
  return {
    strip(chunkText) {
      if (leadResolved) {
        return processTagged(chunkText);
      }

      leadBuffer += (chunkText || '');
      const closeIdx = leadBuffer.indexOf(CLOSE);
      const openIdx = leadBuffer.indexOf(OPEN);

      if (closeIdx !== -1 && (openIdx === -1 || closeIdx < openIdx)) {
        // A closing tag showed up with no opening tag before it: the model's
        // reasoning was primed silently by the server. Drop everything up to
        // and including this "</think>" and treat the rest normally.
        const rest = leadBuffer.slice(closeIdx + CLOSE.length);
        leadResolved = true;
        leadBuffer = '';
        return processTagged(rest);
      }

      if (openIdx !== -1 || leadBuffer.length >= leadBufferLimit) {
        // Either a normal, explicit opening tag showed up (hand off to the
        // regular state machine, which already knows how to hide content
        // until the matching close), or we've held enough text with no tag
        // at all to be confident this is just an ordinary reply.
        const buffered = leadBuffer;
        leadResolved = true;
        leadBuffer = '';
        return processTagged(buffered);
      }

      // Still undecided — hold this back for now, nothing to send yet.
      return '';
    },
    flush
  };
  ...
  try {
    ...
    if (stream) {
          ...
          lines.forEach(line => {
            if (line.startsWith('data: ')) {
              if (line.includes('[DONE]')) {
                const trailing = stripInlineThink.flush();
                if (trailing) {
                  res.write(`data: ${JSON.stringify({ choices: [{ index: 0, delta: { content: trailing }, finish_reason: null }] })}\n\n`);
                }
                res.write('data: [DONE]\n\n');
                return;
              }
              ...
              try {
                ...
                if (data.choices?.[0]?.delta) {
                  ...
                  if (SHOW_REASONING) {
                    ...
                    data.choices[0].delta.content = stripInlineThink.strip(content || '');
                  }
      ...
      response.data.on('end', () => {
        // Fallback in case the upstream stream closed without ever sending
        // a [DONE] line: make sure anything still buffered gets flushed.
        const trailing = stripInlineThink.flush();
        if (trailing && !res.writableEnded) {
          res.write(`data: ${JSON.stringify({ choices: [{ index: 0, delta: { content: trailing }, finish_reason: null }] })}\n\n`);
        }
        res.end();
      });
